...
bool cRoute::Read(const char *fileName)
{
	...
	//		m_path.push_back(gis::Vector2d(atof(lon), atof(lat)));
	//	}
	//}
...
bool cRoute::GeneratePath(const float GAP)
{
	...
	while (loopCnt < 1000)
	{
		...
		{
			for (int i = 0; i < 4; ++i)
				m_genPath.push_back(gis::Vector2d(rect[i].x, rect[i].z));
		}
		...
		{
			saveRoute = true;
			m_genPath.push_back(gis::Vector2d(out.x, out.y));
		}
		else
		{
			gap /= 2.f;
			...
			if (gap < 0.0003f)
			{
				m_genPath.push_back(gis::Vector2d(center.x, center.z));
				break; // exit
			}
		}
	}
