...
bool cQuadTileManager::Update(graphic::cRenderer &renderer, cTerrainQuadTree &terrain
	, const gis::Vector2d &camLonLat, float deltaSeconds)
{
...
void cQuadTileManager::SortingDownloadFile(const gis::Vector2d &camLonLat, cTerrainQuadTree &terrain
	, const float deltaSeconds)
{
	...
	{
	public:
		cSortTask(const gis::Vector2d &lonLat) : m_lonLat(lonLat) {}
		gis::Vector2d m_lonLat;

		virtual void Sorting(vector<cTask*> &tasks) override
		{
			struct CompareFacility
			{
				CompareFacility(const gis::Vector2d &lonLat) : mLonLat((float)lonLat.x, 0, (float)lonLat.y) {}
				Vector3 mLonLat;
...
void cQuadTileManager::UpdateLoadModelFile(graphic::cRenderer &renderer
	, const gis::Vector2d &camLonLat, cTerrainQuadTree &terrain
	, const float deltaSeconds
)
{
	...
	{
		sModelCompare(const gis::Vector2d &lonLat) : mLonLat((float)lonLat.x, 0, (float)lonLat.y) {}
	...
	, const sReadyLoadModel &modInfo)
{
	...
	gis::Vector2d lonLat = tile->m_facilityIndex->m_objs[i].centerPos;
