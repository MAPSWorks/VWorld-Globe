...
)
{
	...
	tfm.pos.y = 0.f;
	renderer.m_cbPerFrame.m_v->mWorld = m_isSphereRender? XMMatrixIdentity() : XMMatrixTranspose(tfm.GetMatrixXM());
	...
	if (m_isSphereRender)
	{
		//const Vector3 n0 = gis::GetNormal(gis::TileLoc2WGS84(m_level, m_loc.x, m_loc.y + 1));
		//const Vector3 n1 = gis::GetNormal(gis::TileLoc2WGS84(m_level, m_loc.x + 1, m_loc.y + 1));
		//const Vector3 n2 = gis::GetNormal(gis::TileLoc2WGS84(m_level, m_loc.x, m_loc.y));
		//const Vector3 n3 = gis::GetNormal(gis::TileLoc2WGS84(m_level, m_loc.x + 1, m_loc.y));
		//renderer.m_cbTessellation.m_v->vtxDir[0] = n0;
		//renderer.m_cbTessellation.m_v->vtxDir[1] = n1;
		//renderer.m_cbTessellation.m_v->vtxDir[2] = n2;
		//renderer.m_cbTessellation.m_v->vtxDir[3] = n3;
		const Vector2 lonLat0 = gis::TileLoc2WGS84(m_level, m_loc.x, m_loc.y + 1).ToVector2();
		const Vector2 lonLat1 = gis::TileLoc2WGS84(m_level, m_loc.x + 1, m_loc.y + 1).ToVector2();
		const Vector2 lonLat2 = gis::TileLoc2WGS84(m_level, m_loc.x, m_loc.y).ToVector2();
		const Vector2 lonLat3 = gis::TileLoc2WGS84(m_level, m_loc.x + 1, m_loc.y).ToVector2();
		renderer.m_cbTessellation.m_v->vtxDir[0] = Vector4(ANGLE2RAD(lonLat0.x), ANGLE2RAD(lonLat0.y)
			, ANGLE2RAD(lonLat1.x), ANGLE2RAD(lonLat1.y));
		renderer.m_cbTessellation.m_v->vtxDir[1] = Vector4(ANGLE2RAD(lonLat2.x), ANGLE2RAD(lonLat2.y)
			, ANGLE2RAD(lonLat3.x), ANGLE2RAD(lonLat3.y));
	}
...
)
{
	...
	//	//if (dist < (eyePos.y * 3))
	//		RenderPoi(renderer, deltaSeconds, tm);
	//}
	...
	if (isShowPoi1 && m_poi[1])
	{
		...
		for (auto &poi : m_poi[1]->m_pois)
		{
			...
			// 카메라가 수평선에 놓여있을 때,
			// 카메라에서 먼 위치에 있는 지역명은 보이지 않게한다.
			const Vector3 dir = (poi.relPos - camPos).Normal();
			const float dot = dir.DotProduct(Vector3(0, -1, 0));
			if (dot < 0.1f)
			{
				if (200.f < (poi.relPos - camPos).Length())
					continue;
			}
