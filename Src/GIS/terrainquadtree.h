...
public:
	...
	void Update(graphic::cRenderer &renderer, const gis::Vector2d &camLonLat, const float deltaSeconds);
	...
	gis::Vector2d GetLongLat(const Ray &ray);
	Vector3 Get3DPos(const gis::Vector2d &lonLat);
	gis::Vector2d GetWGS84(const Vector3 &relPos);
...
protected:
	...
	void RenderTessellation(graphic::cRenderer &renderer, const float deltaSeconds
		, const graphic::cFrustum &frustum);
	void RenderFacility(graphic::cRenderer &renderer, const float deltaSeconds
		, const graphic::cFrustum &frustum);
	void RenderQuad(graphic::cRenderer &renderer, const float deltaSeconds
		, const graphic::cFrustum &frustum, const Ray &ray);
	void RenderResDistribution(graphic::cRenderer &renderer, const float deltaSeconds
		, const graphic::cFrustum &frustum, const int resType);
	void RenderRect3D(graphic::cRenderer &renderer, const float deltaSeconds
		, const sRectf &rect, const graphic::cColor &color);
	void RenderRect3D(graphic::cRenderer &renderer, const float deltaSeconds
		, const gis::sBoundingRect &rect, const graphic::cColor &color);
	inline bool IsContain(const graphic::cFrustum &frustum, const sRectf &rect, const float maxH);
	inline bool IsContain(const graphic::cFrustum &frustum, const gis::sBoundingRect &brect, const float maxH);
